...
**Cleanix** is a next-generation, AI-driven municipal solid waste management platform designed to eliminate overflowing bins, optimize collection vehicle fuel consumption, and provide a unified ground-truth feedback loop between administrators, collection drivers, and field survey teams.
...
```
         ...
         ▼                                  ▼                                  ▼
┌──────────────────┐               ┌──────────────────┐               ┌──────────────────┐
│  🛡️ ADMIN PORTAL  │               │ 🚚 DRIVER PORTAL  │               │ 📋 SURVEY PORTAL │
│     (/admin)     │               │     (/driver)    │               │     (/survey)    │
├──────────────────┤               ├──────────────────┤               ├──────────────────┤
...
### 1. 🛡️ Municipal Admin Command Center (`/admin`)
...
sequenceDiagram
    autonumber
    actor Admin as 🛡️ Municipal Admin
    actor Driver as 🚚 Truck Driver
    actor Surveyor as 📋 Survey Team
    participant ML as 🧠 ML Prediction Engine
    participant ORTools as ⚙️ Google OR-Tools CVRP
    participant DB as 🗄️ Database (PostgreSQL / SQLite)
    participant OSRM as 🗺️ OpenStreetMap / OSRM
...
| **Frontend UI** | React 19 + Vite + Tailwind CSS | Ultra-fast client build with role-guarded routing and glassmorphic dark theme. |
...
| :--- | :--- | :--- | :--- |
| 🛡️ **Admin Portal** | `admin@sih.com` | `admin123` | Full municipal control, ML trigger, OR-Tools optimizer, hotspot approval. |
