...
def fetch_osrm_geometry(ordered_stops: List[Dict]) -> Dict:
    """
    Fetches actual road polyline GeoJSON coordinates between ordered stops.
    """
    ...
    try:
        ...
        print(f"[Warning] Geometry fetch failed: {e}. Using straight lines.")

    # Fallback: straight lines between coordinates
    return {
        "type": "LineString",
...
def format_cluster_routes(manager, routing, solution, nodes: List[Dict], cluster_id: str, global_truck_counter: int) -> List[Dict]:
    """
    Formats solver output into structured stops and GeoJSON polylines for React Leaflet.
    """
    if not solution:
        return []
    ...
    for vehicle_id in range(manager.GetNumberOfVehicles()):
        ...
        while not routing.IsEnd(index):
            ...
            action = "START" if is_depot else ("COLLECT_AND_TAKE_PHOTO" if is_hotspot else "COLLECT_WASTE")
        ...
        # Only register routes that visited at least one collection point
